exports.run = async (client, message, args, level) => { // eslint-disable-line no-unused-vars
  ...
  if (Math.round(client.musicDispatcher.volume*50) >= 100) {
    return message.channel.send(`Volume: ${Math.round(client.musicDispatcher.volume*50)}%`);
  }

  client.musicDispatcher.setVolume(Math.min((client.musicDispatcher.volume*50 + (2*(message.content.split('+').length-1)))/50,2));
...
exports.help = {
  name: "volume+",
  category: "Music",
  description: "Increase Avaline's volume output.",
  usage: "volume+"
};